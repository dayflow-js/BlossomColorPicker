...
export class ChromePicker {
  ...
  private render(): void {
    ...
    const slidersContainer = createElement(
      'div',
      {},
      {
    ...
    const alphaContainer = createElement(
      'div',
      {},
      { class: 'bcp-chrome-slider-container' }
    );
    this.alphaSliderTrack = createElement(
      'div',
      {},
      { class: 'bcp-chrome-alpha-gradient' }
    );
    this.alphaSliderHandle = createElement(
      'div',
      {},
      { class: 'bcp-chrome-slider-handle' }
    );
    this.alphaSliderInput = createElement(
      'input',
      {},
      {
        type: 'range',
    ...
    this.inputsEl = createElement(
      'div',
      {},
      { class: 'bcp-chrome-inputs-container' }
    );
  ...
  private renderRgbaInputs(color: BlossomColorPickerColor): void {
    const r = this.createInputGroup('R', String(color.r), () =>
      this.syncRgba(color)
    );
    const g = this.createInputGroup('G', String(color.g), () =>
      this.syncRgba(color)
    );
    const b = this.createInputGroup('B', String(color.b), () =>
      this.syncRgba(color)
    );
    const a = this.createInputGroup(
      'A',
      color.alpha === 100 ? '1' : (color.alpha / 100).toFixed(2),
      () => this.syncRgba(color)
    );
  ...
  private renderHslaInputs(color: BlossomColorPickerColor): void {
    const h = this.createInputGroup('H', String(Math.round(color.hue)), () =>
      this.syncHsla(color)
    );
    const s = this.createInputGroup(
      'S',
      String(Math.round(color.saturation)),
      () => this.syncHsla(color)
    );
    const l = this.createInputGroup(
      'L',
      String(Math.round(color.lightness ?? 50)),
      () => this.syncHsla(color)
    );
    const a = this.createInputGroup(
      'A',
  ...
  private applyTheme(): void {
    ...
    for (const [key, value] of Object.entries(
      this.opts.darkModeColors
    ) as Array<[keyof ChromePickerThemeColors, string | undefined]>) {
      if (!value) continue;
